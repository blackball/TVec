/**
 * Simple inline implementation of general dynamic array in C.
 *
 * @blackball
 */
...
#define TVEC_HEADER_SIZE     8
#define TVEC_HEADER_SIZE_I   0
#define TVEC_HEADER_ALLOC_I  1
...
#define TVEC_NEW(V, sz)                                                 \
    do {                                                                \
        ...
        (V) = (TVec *)malloc(sizeof(TVec) * (TVEC_HEADER_SIZE + s));    \
        if ((V) == NULL) {                                              \
            exit(-1);                                                   \
        }                                                               \
        (V)[TVEC_HEADER_SIZE_I] = 0;                                    \
...
#define TVEC_RESIZE(V, nsz)                                             \
    do {                                                                \
        if ( TVEC_ALLOC(V) != (nsz)) {                                  \
            (V) = (TVec *)realloc((V), sizeof(TVec) * (nsz + TVEC_HEADER_SIZE)); \
            if ((V) == NULL) {                                          \
                exit(-1);                                               \
            }                                                           \
            TVEC_ALLOC(V) = (nsz);                                      \
...
#define TVEC_SORT(V, CMP)                       \
    do {                                        \
        TVec *data = TVEC_DATAPTR(V);           \
        const int size = TVEC_SIZE(V);          \
        _QSORT(TVec, data, size, CMP);          \
    } while(0)
